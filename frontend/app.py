...
def api(method: str, path: str, *, json_body=None, auth=True) -> dict | None:
    ...
    try:
        r = httpx.request(
            method, f"{BACKEND}{path}", json=json_body, headers=headers, timeout=20
        )
        ...
        if st.session_state.user:
            ...
            if st.button("🚪 Logout", use_container_width=True):
                for k in (
                    ...
                    st.session_state[k] = (
                        None if k in ("token", "user", "active_session_id") else []
                    )
            ...
            if result:
                ...
                try:
                    ...
                    if r.status_code == 403 and "not verified" in r.text.lower():
                        ...
                        otp_res = api(
                            "POST", "/otp/send", json_body={"email": email}, auth=False
                        )
                        st.session_state.pending_verify_email = email
                        st.session_state.otp_demo_code = (
                            otp_res.get("otp_code") if otp_res else None
                        )
        ...
        if submitted:
            if password != password2:
                ...
                if result:
                    ...
                    st.session_state.pending_verify_email = email
                    st.session_state.otp_demo_code = (
                        otp_result.get("otp_code") if otp_result else None
                    )
                    st.session_state.page = "verify_otp"
                    st.rerun()
...
def page_dashboard():
    ...
    # ── Fetch Data from API (Polling) ──────────────────────────────
    try:
        r = httpx.get(f"{BACKEND}/api/readings/recent?limit=60", timeout=5)
        if r.status_code == 200:
            fresh = r.json()
            st.session_state.data_buffer = deque(
                sorted(fresh, key=lambda x: x["timestamp"]),
                maxlen=MAX_POINTS,
            )
            st.success(f"🟢 Live — {len(fresh)} readings loaded")
        else:
            st.warning(f"API returned {r.status_code}")
    except Exception as e:
        st.error(f"❌ Cannot reach backend: {e}")

    data = list(st.session_state.data_buffer)

    # ── Empty State ────────────────────────────────────────────────
    if not data:
        st.info("No data yet — start the simulator")
        st.code("python -m simulator.data_simulator --all-machines")
        return
    ...
    def chart(title, values, ylabel, color="#1f77b4"):
        colors = ["red" if anomalies[i] else color for i in range(len(values))]
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(
                x=timestamps,
    ...
    c1.plotly_chart(
        chart("🌡 Temperature", [d["temperature"] for d in data], "°C"),
        use_container_width=True,
    )

    c2.plotly_chart(
        chart("📊 Pressure", [d["pressure"] for d in data], "kPa", "#ff7f0e"),
        use_container_width=True,
    )

    c1.plotly_chart(
        chart("📳 Vibration", [d["vibration"] for d in data], "mm/s", "#9467bd"),
        use_container_width=True,
    )

    c2.plotly_chart(
        chart("🔴 Defect Rate", [d["defect_rate"] * 100 for d in data], "%", "#d62728"),
        use_container_width=True,
    )
    ...
    fig_spd.add_trace(
        go.Scatter(
            x=timestamps,
            y=[d["production_speed"] for d in data],
            fill="tozeroy",
            mode="lines",
...
def page_chat():
    ...
    with col_right:
        ...
        prompt = st.chat_input(
            "Ask about quality issues, root causes, recommendations…"
        )
        if prompt:
            # Optimistic UI
...
def page_profile():
    ...
    tab_info, tab_pw, tab_delete = st.tabs(
    ...
    with tab_info:
        ...
        if submitted:
            result = api(
                "PUT",
                "/users/me",
                json_body={"full_name": full_name, "username": username},
            )
            if result:
...
def page_admin():
    ...
    for u in users:
        with st.expander(
            f"{'🔴' if not u['is_active'] else '🟢'} {u['username']} — {u['email']} ({u['role']})"
        ):
